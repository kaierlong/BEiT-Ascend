...
def cosine_lr(args, batch_num):
    ...
    if args.warmup_length > 0:
        min_lrs.append(args.warmup_lr)
    if args.cool_length > 0:
        min_lrs.append(args.cool_lr)
    min_lr = min(min_lrs)
    ...
    for epoch in range(args.epochs - args.cool_length):
        for batch in range(batch_num):
            learning_rate.append(_lr_adjuster(epoch + batch / batch_num))

    for epoch in range(args.cool_length):
        for _ in range(batch_num):
            learning_rate.append(args.cool_lr)

    learning_rate = np.clip(learning_rate, a_min=min_lr, a_max=args.base_lr)
    return learning_rate
